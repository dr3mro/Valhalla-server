...
namespace api::v2::Http
{
    ...
    inline std::string getDescription(Status code)
    {
        ...
        static const std::unordered_map<Status, std::string> StatusDescriptions = {{CONTINUE, "Continue"}, {SWITCHING_PROTOCOLS, "Switching Protocols"},
            ...
            {SEE_OTHER, "See Other"}, {NOT_MODIFIED, "Not Modified"}, {TEMPORARY_REDIRECT, "Temporary Redirect"}, {PERMANENT_REDIRECT, "Permanent Redirect"},
            {BAD_REQUEST, "Bad Request"}, {UNAUTHORIZED, "Unauthorized"}, {PAYMENT_REQUIRED, "Payment Required"}, {FORBIDDEN, "Forbidden"},
            {NOT_FOUND, "Not Found"}, {METHOD_NOT_ALLOWED, "Method Not Allowed"}, {NOT_ACCEPTABLE, "Not Acceptable"}, {CONFLICT, "Conflict"}, {GONE, "Gone"},
            {LENGTH_REQUIRED, "Length Required"}, {PRECONDITION_FAILED, "Precondition Failed"}, {PAYLOAD_TOO_LARGE, "Payload Too Large"},
            {UNSUPPORTED_MEDIA_TYPE, "Unsupported Media Type"}, {INTERNAL_SERVER_ERROR, "Internal Server Error"}, {NOT_IMPLEMENTED, "Not Implemented"},
