...
template <Appointment_t T>
void AppointmentController<T>::Update(CALLBACK_&& callback, const Requester&& requester, std::string_view data, const std::optional<uint64_t> id)
{
    EntityController<T>::Update(std::move(callback), std::move(requester), data, id);
}

template <Appointment_t T>
void AppointmentController<T>::Delete(CALLBACK_&& callback, const Requester&& requester, const std::optional<uint64_t> id)
{
...
#define INSTANTIATE_APPOINTMENT_CONTROLLER(TYPE)                                                                                                             \
    ...
    template void AppointmentController<TYPE>::Update(CALLBACK_&& callback, const Requester&& requester, std::string_view data, std::optional<uint64_t> id); \
    template void AppointmentController<TYPE>::Delete(CALLBACK_&& callback, const Requester&& requester, std::optional<uint64_t> id);                        \
