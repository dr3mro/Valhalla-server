...
class ClinicController : public EntityController<T>, public ClinicControllerBase
{
   private:
    ...
    template <typename U = T>
    void GetVisitsImpl(CALLBACK_ &&callback, const Requester &&requester, std::optional<uint64_t> id)
        requires(std::is_same<U, Patient>::value);
   ...
   public:
    ...
    void GetVisits(CALLBACK_ &&callback, const Requester &&requester, std::optional<uint64_t> id) final;
};