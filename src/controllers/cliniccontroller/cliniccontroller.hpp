...
class ClinicController : public EntityController<T>, public ClinicControllerBase
{
   private:
    ...
    template <typename U = T>
    void DeleteImpl(CALLBACK_ &&callback, const Requester &&requester, const std::optional<uint64_t> id)
        requires(std::is_same<U, Patient>::value || std::is_same<U, Visits>::value)
    {
    ...
    template <typename U = T>
    void DeleteImpl(CALLBACK_ &&callback, [[maybe_unused]] const Requester &&requester, const std::optional<uint64_t> id)
        requires(!std::is_same<U, Patient>::value && !std::is_same<U, Visits>::value)
    {
        (void)id;
    ...
    template <typename U = T>
    void SearchImpl(CALLBACK_ &&callback, [[maybe_unused]] const Requester &&requester, [[maybe_unused]] std::string_view data)
        requires(!std::is_same<U, Patient>::value)
    {
    ...
    template <typename U = T>
    void GetVisitsImpl(CALLBACK_ &&callback, const Requester &&requester, const std::optional<uint64_t> id)
        requires std::is_same<U, Patient>::value
    {
        Http::Error error;
        try
    ...
    // Only enable GetVisits if T is of type Patient
    template <typename U = T>
    void GetVisitsImpl(CALLBACK_ &&callback, [[maybe_unused]] const Requester &&requester, const std::optional<uint64_t> id)
        requires(!std::is_same<U, Patient>::value)
    {
        (void)id;
   ...
   public:
    ...
    void Update(CALLBACK_ &&callback, const Requester &&requester, std::string_view data, const std::optional<uint64_t> id) final
    {
        // Update for all clinic entities
        EntityController<T>::Update(std::move(callback), std::move(requester), data, id);
    }

    void Delete(CALLBACK_ &&callback, const Requester &&requester, const std::optional<uint64_t> id) final
    {
    ...
    void GetVisits(CALLBACK_ &&callback, const Requester &&requester, const std::optional<uint64_t> id) final
    {
