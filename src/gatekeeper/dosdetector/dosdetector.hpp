...
class DOSDetector
{
   ...
   private:
    ...
    inline void __attribute((always_inline)) clean_requests(
        const std::chrono::time_point<std::chrono::steady_clock> &window);
    inline void __attribute((always_inline)) clean_ratelimited_ips(
        const std::chrono::time_point<std::chrono::steady_clock> &now);
    inline void __attribute((always_inline)) clean_banned_ips(
        const std::chrono::time_point<std::chrono::steady_clock> &now);

    //                     // IP                       // Hash of Request //
    //                     times of requests
    std::unordered_map<std::string, std::unordered_map<std::string, std::deque<std::chrono::steady_clock::time_point>>>
                                                                           requests_;
    ...
    void                                                           cleanUpTask();
    inline std::optional<std::string> __attribute((always_inline)) generateRequestFingerprint(
        const DOSDetector::Request &request);
    ...
    inline bool __attribute((always_inline)) regexFind(std::string_view                       remote_ip,
                                                       const std::unordered_set<std::string> &list, std::mutex &mtx);
